  ...
  <div>
    <template v-for="(tab, index) in menu1Sub1Tab3Page2Store.tabList" :key="tab">
      <ActiveButton
        ref="childRefs"
        :title="tab.name"
        :is-active="menu1Sub1Tab3Page2Store.selectedTabId === tab.id"
...
<script setup lang="ts">
  ...
  import {
    Menu1Sub1Tab3Page2TabIdType,
    useMenu1Sub1Tab3Page2Store,
  } from '@/stores/app/menu-1/sub-1/tab-C/page-2'

  const menu1Sub1Tab3Page2Store = useMenu1Sub1Tab3Page2Store()

  const childRefs = ref<ActiveButton[]>([])

  function handleCustomEvent(isActive: boolean, id: Menu1Sub1Tab3Page2TabIdType, index: number) {
    if (!isActive) return
    ...
    menu1Sub1Tab3Page2Store.selectTab(id)
  }
