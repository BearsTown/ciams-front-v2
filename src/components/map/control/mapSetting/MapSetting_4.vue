...
<script lang="ts" setup>
  ...
  const activeCodeKey = ref<BaseMap>()
  ...
  //   return Object.entries(baseMaps.value[activeCodeKey.value!].types).find(
  //     ([_, type]) => type.checked,
  //   )?.[0]
  // })
  ...
  const activeCodeKey = computed({
    ...
    set(newKey: string) {
      const types = baseMaps.value[activeCodeKey.value!].types
