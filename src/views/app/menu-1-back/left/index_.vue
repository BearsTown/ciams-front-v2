...
<script setup lang="ts">
  ...
  import StepB from '@/components/app/menu-1/sub-1/left/Tab-B.vue'
  import StepC from '@/components/app/menu-1/sub-1/left/Tab-C.vue'
  ...
  const currentTab = ref('StepA')
  const tabList = [
    { name: '용도지역현황', id: 'StepA', component: StepA },
    { name: '수립대상지역', id: 'StepB', component: StepB },
    { name: '대상지현황', id: 'StepC', component: StepC },
  ]
  ...
  const mapWrap = globalStore.getMapInstance(MapType.MAP_1)
  ...
  function handleTabChange(tabId: string) {
    ...
    switch (currentTab.value) {
      ...
      case 'StepA': {
        ...
        globalStore.layoutSelected?.right?.visible.off()

        // mapWrap.value.layerGroupVisible(LayerGroupType.MAP_1_LAYER_GROUP_1, true)
      ...
      case 'StepB': {
        ...
        // mapWrap.value.layerGroupVisible(LayerGroupType.MAP_1_LAYER_GROUP_2, true)
      ...
      case 'StepC': {
        // isActiveTimePoint.value = false
        globalStore.layoutSelected?.right?.visible.on()
        globalStore.layoutSelected?.right?.collapse.on()

        // mapWrap.value.layerGroupVisible(LayerGroupType.MAP_1_LAYER_GROUP_3, true)
