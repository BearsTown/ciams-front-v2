...
<script setup lang="ts">
  ...
  import PieChartItem from '@/components/app/classify-analysis/PieChartItem.vue'
  import AsyncImage from '@/views/app/classify-analysis/comprehensive/right/AsyncImage.vue'
  import AnalysisTable from '@/views/app/classify-analysis/management/right/AnalysisTable.vue'
  ...
  const state = classifyAnalysisComprehensiveStore.state
  const dialog = ref<boolean>(false)
  const info = ref<ClassComprehensiveDto.Info>()
  ...
  watch(
    ...
    async () => {
      if (overview.value?.zoneNo) {
        const { data } = await getComprehensiveInfo(overview.value?.zoneNo)
        info.value = data
      } else {
