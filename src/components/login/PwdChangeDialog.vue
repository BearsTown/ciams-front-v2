<template>
  <CustomModal @close="closeDialog" v-if="dialogVisible" class="pwChangeModal">
    ...
    <div class="form">
      <div :class="validState['currentPwd'] ? 'form-group has-error' : 'form-group'">
        ...
        <input
          ...
          autocomplete="off"
          maxlength="50"
          @blur="validate('currentPwd', true)"
        />

        <div class="form-feedback" v-if="validate('currentPwd') != ''">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
          ...
          <span class="feedback-text">{{ validate('currentPwd') }}</span>
        </div>
      </div>

      <!-- error validation class: has-error -->
      <!-- success validation class: has-success -->
      <div :class="validState['pass'] ? 'form-group has-error' : 'form-group'">
        <!-- error validation class: has-error -->
        ...
        <div class="form-feedback" v-if="validate('pass') != ''">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
          ...
          <span class="feedback-text">{{ validate('pass') }}</span>
        </div>
      ...
      <div :class="validState['checkPass'] ? 'form-group has-error' : 'form-group'">
        <!-- error validation class: has-error -->
        ...
        <div class="form-feedback" v-if="validate('checkPass') != ''">
          <svg
            width="18"
            height="18"
            viewBox="0 0 18 18"
          ...
          <span class="feedback-text">{{ validate('checkPass') }}</span>
        </div>
      </div>
...
<script setup lang="ts">
  ...
  function validate(key, init?: boolean) {
    ...
    let value = pwdForm[key].trim()
    if ('currentPwd' == key && validState[key]) {
      if (value.length == 0) return '필수 정보입니다'
      else validState[key] = false
    } else if ('pass' == key && validState[key]) {
      if (value.length == 0) return '필수 정보입니다'
      else if (!regExp_password.test(value))
        return '비밀번호는 8~20자리 영문 대·소문자, 숫자 및 특수문자를 조합해주세요.'
      else validState[key] = false
    } else if ('checkPass' == key && validState[key]) {
      if (value.length == 0) return '필수 정보입니다'
      else if (pwdForm['pass'] != pwdForm[key]) return '새 패스워드와 일치하지 않습니다.'
      else validState[key] = false
    }
    return ''
  }
...
<style>
  @import '@/assets/css/pwChangeModal.css';
</style>
