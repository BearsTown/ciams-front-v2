...
export const useMenu1_2_3Store = defineStore('useMenu1-2-3Store', () => {
  ...
  async function callChangeYear() {
    ...
    if (selectDetail && state.activeYear && state.activeDistId) {
      ...
      columns.value = await test_getMenu1_2_3Data()

      await loadIndustryFeatures()

      // const columns = await test_getMenu1_2_3Data()

      // selectDetail!.tableData = markRaw(
      //   dataUtil.calculateRatios(data, state.attributes, { round: 1 }),
      // )

      // dataUtil.processData(lay)

      const fLayer = selectDetail?.layer?.getLayer() as UitWFSLayer
      if (fLayer) {
        selectDetail.tableData = dataUtil.processData(
          fLayer.getSource().getFeatures(),
          columns.value,
          state.attributes,
          'CLS1',
          false,
  ...
  const dynamicStyle = function (feature) {
    ...
    const propertyValue = feature.get('CLS1') // Feature의 'type' 속성 가져오기

    // const color = columns.value.find((col) => col.name === propertyValue)
    let column = columns.value.find((col) => col.name === propertyValue)
    if (!column) {
      column = columns.value.find((col) => col.name === 'other')
    }
