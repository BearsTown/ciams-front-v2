<template>
  <div>
    <template v-for="(tab, index) in basicLocIndStatusIndustrialStore.tabList" :key="tab">
      <ActiveButton
        ref="childRefs"
        :title="tab.name"
        :is-active="basicLocIndStatusIndustrialStore.selectedTabId === tab.id"
...
<script setup lang="ts">
  ...
  function handleCustomEvent(
    isActive: boolean,
    id: BasicLocIndStatusIndustrialTabIdType,
    index: number,
  ) {
    if (!isActive) return
    ...
    basicLocIndStatusIndustrialStore.selectTab(id)
  }
</script>
