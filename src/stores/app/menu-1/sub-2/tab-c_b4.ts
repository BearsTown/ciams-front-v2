import { computed, markRaw, reactive, Ref, ref, shallowRef, toRefs } from 'vue'
...
export const useMenu1_2_3Store = defineStore('useMenu1-2-3Store', () => {
  ...
  async function fetchAttributes() {
    ...
    state.attributes.value = rawAttributes
  ...
  async function fetchCategories() {
    rawCategories = await test_getMenu1_2_3()

    rawCategories.forEach((category) => {
      category.years = category.years.map((year) => ({
        id: year,
        title: `${year}년`,
        layer: null,
        features: [],
      ...
      category.active = useBoolean(false)
    })

    state.categories.value = rawCategories.map((category) => ({
      ...category,
      active: useBoolean(true),
      years: category.years.map((year) => ({
        year: year.id,
        title: `${year.id}년`,
      })),
  ...
  //   if (select) {
  //     return select.years.find((detail) => detail.year === state.activeYear)
  //   }
  // }
  ...
  function findSelectRef() {
    if (!commonUtil.isEmpty(state.categories)) {
      return state.categories.value.find((category) => category.active.status.value)
    } else {
      return undefined
    }
    // if (
  ...
  function findSelect() {
    if (!commonUtil.isEmpty(state.categories)) {
      // if (!commonUtil.isEmpty(rawCategories)) {
      return rawCategories.find((category) => category.active.status)
      // return rawCategories.find((category) => category.active.status.value)
      // return state.categories.value.find((category) => category.active.status)
      // return state.categories.value.find((category) => {
      //   return category.active.status ? rawCategories.find((c) => c.id === category.id) : []
      // })
    } else {
      return undefined
    }
  ...
  async function setActiveCategory(categoryId: string) {
    const categoryExists = state.categories.value.some((category) => category.id === categoryId)
    if (!categoryExists) {
    ...
    state.categories.value.forEach((category) => {
  ...
  async function callChangeYear() {
    ...
    selectDetail!.tableData = dataUtil.calculateRatios(data, state.attributes.value, { round: 1 })
  ...
  async function loadIndustryFeatures() {
    ...
    let text2 = await points.text()
    text2 = text2.replace(/\n/gi, '\\r\n')
    const features2 = new GeoJSON().readFeatures(text2) as Feature[]

    const fLayer = getSelectDetail()?.layer?.getLayer() as UitWFSLayer
    fLayer.clear()
    fLayer.addFeatures(features2)
  }
  ...
  const layerVisible = computed({
    get() {
      return getSelectDetail()?.layer?.isVisible()
    },
    set(visible) {
