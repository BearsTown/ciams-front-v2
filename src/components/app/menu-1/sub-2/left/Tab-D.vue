...
<script setup lang="ts">
  ...
  async function handleSwitchChange(categoryId) {
    await menu1_2_4Store.setActiveCategory(categoryId)
  }

  async function zoneItemSelect(item: GisCiamsDistDTO.Search.Row) {
    await menu1_2_4Store.callSelectDist(item.distNo)
    // await menu1_2_3Store.loadDistFeatures(item.distNo)
  }

  function clear() {
    // overview.value = null
    menu1_2_4Store.distVectorLayer.clear()
  }
  ...
  onBeforeMount(async () => {
    ...
    await menu1_2_4Store.init(3)

    console.log(state.categories)
    load()

    // menu1_2_3Store.state.categories[0].active.on()
    await handleSwitchChange(menu1_2_4Store.state.categories[0].id)
  })
  ...
  onActivated(() => {
    ...
    menu1_2_4Store.mapWrap.value?.setViewLayersVisible(layerGroupName!, true)
  })
</script>
