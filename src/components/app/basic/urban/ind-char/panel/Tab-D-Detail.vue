<template>
  <div class="detail">
    ...
    >
      <el-tab-pane
        v-for="data in basicUrbanIndCharStore.years"
        :label="`${data.title}`"
        :name="data.id"
        :key="data.id"
    ...
    <div class="top">
      <div style="display: flex; padding-bottom: 5px">
        ...
        <el-switch
          size="small"
          v-model="basicUrbanIndCharStore.layerVisible"
          :disabled="!basicUrbanIndCharStore.getSelectDetail()"
        />
      </div>
...
<script setup lang="ts">
  ...
  const filteredTableData = computed(() => {
    // return basicUrbanIndCharStore.getSelectDetail()?.tableData
    return basicUrbanIndCharStore.getSelectDetail()?.tableData?.filter((row) => row.visible)
  })

  const filteredChartData = computed(() => {
    return basicUrbanIndCharStore.getSelectDetail()?.chartData
    // return basicUrbanIndCharStore.getSelectDetail()?.chartData.filter((row) => row.visible)
  })

  function tabChangeListener(year) {
    // basicUrbanIndCharStore.callChangeYear()
    basicUrbanIndCharStore.updateYear(year)
  }
